...
module.exports = {
  ...
  // Billing-portal session for a customer (admin opens it on their behalf).
  createPortalSession: function (customer, returnUrl) { return call("POST", "/v1/billing_portal/sessions", { customer: customer, return_url: returnUrl }); },
  // Create a default portal configuration (needed once in test mode before the
  // portal can be used).
  createPortalConfiguration: function (cfg) { return call("POST", "/v1/billing_portal/configurations", cfg); },
  // Cancel at period end (keeps access through the paid period).
  cancelAtPeriodEnd: function (id) { return call("POST", "/v1/subscriptions/" + encodeURIComponent(id), { cancel_at_period_end: true }); }
};
